...
function toSearchCard(
  ...
  const cleanName = info.clean_name?.trim();
  const setId = info.set_id ?? null;
  return {
    id: card.id,
    name: cleanName !== undefined && cleanName !== "" ? cleanName : info.name ?? "Unknown card",
...
/**
 * First TCGplayer market price across printing variants. CardMarket-only
 * cards report EUR, which would lie next to the app's $ labels, so they
 * stay priceless instead.
 */
